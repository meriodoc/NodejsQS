...
// Query from bilal start

var now = DateTime.Now;

          var q = metadata.qs_Product.Where(p => p.ProductGroup == "Hamper" && !p.isObsolete
                  && p.isSellOnline == true && p.isNeverDisminishing == false && p.isConstrained != true).OrderBy(p => p.DisplayGroupID).ThenBy(p => p.DisplayOrder).ThenBy(p => p.DisplayDescrip).Select(p =>
                  new bl_Product
                  {
                      ProductID = p.ProductID,
                      ProductCode = p.ProductCode,
                      DisplayDescrip = p.DisplayDescrip,
                      ProductDescrip = p.ProductDescrip,
                      ImagePath = p.ImagePath,
                      SellingPrice = p.isTaxableSales == true ? (p.DefaultSellPrice * p.TaxRate) + p.TaxRate : p.DefaultSellPrice,
                      isSpecial = metadata.qs_Special.Any(s => s.ProductID == p.ProductID && s.SpecialFrom >= now && s.SpecialTo <= now)
                  }).ToList();

          return q;

// Query from bilal stop
